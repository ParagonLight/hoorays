...
def splitDatasetWithUsers1(r, ratingFile, dataset, savePrefix):
    dataList = {}
    userIDDict = {}
    itemIDDict = {}
...
#        dataList.append(row)
    ...
    trainSet = utils.createWriteFile(savePrefix + 'splitedItemTrain.dat')
    testSet = utils.createWriteFile(savePrefix + 'splitedItemTest.dat')
    count = 0
    ccount = 0
...
def reverseID(ratingFile):
    newRatingFile = []
    for row in ratingFile:
        strs = row.split("::")
        strs[0], strs[1] = strs[1], strs[0]
        row = '::'.join(strs)
        #newRatingFile.write(row)
        newRatingFile.append(row)
    return newRatingFile
